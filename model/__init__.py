...
class Model(nn.Module):
    ...
    def forward(self, x, idx_scale):
        ...
        if self.self_ensemble and not self.training:
            ...
            # self.model = nn.DataParallel(self.model, range(1))
            # result = self.forward_chop(x)
            #
            # self.model = nn.DataParallel(self.model, range(self.n_GPUs))
            # return result
            return self.model.forward_chop(x)
        else:
            return self.model(x)
    ...
    #     x_h_cut_unfold.cuda()
    #     for i in range(x_range):
    #         y_h_cut_unfold.append(P.data_parallel(self.model, x_h_cut_unfold[i * batchsize:(i + 1) * batchsize, ...],
    #                                               range(self.n_GPUs)).cpu())
    ...
    #     x_w_cut_unfold.cuda()
    #     for i in range(x_range):
    #         y_w_cut_unfold.append(P.data_parallel(self.model, x_w_cut_unfold[i * batchsize:(i + 1) * batchsize, ...],
    #                                               range(self.n_GPUs)).cpu())
    ...
    def forward_chop(self, x, shave=12):
        ...
        x_unfold = torch.nn.functional.unfold(x, padsize, stride=int(shave / 2)).transpose(0, 2).contiguous()
        # var_unfold = torch.nn.functional.unfold(var, padsize, stride=int(shave / 2)).transpose(0, 2).contiguous()

        x_hw_cut = x[..., (h - padsize):, (w - padsize):]
        # var_hw_cut = var[..., (h - padsize):, (w - padsize):]

        # y_hw_cut = self.forward(x_hw_cut.cuda(), var_hw_cut).cpu()
        # _, y_hw_cut = self.forward(x_hw_cut.cuda(), var_hw_cut)
        y_hw_cut, m_hw_cut_list = self.model(x_hw_cut)
        # y_hw_cut = y_hw_cut.cpu()
        # m_hw_cut_list = [m_hw_cut.cpu() for m_hw_cut in m_hw_cut_list]

        x_h_cut = x[..., (h - padsize):, :]
        x_w_cut = x[..., :, (w - padsize):]

        # var_h_cut = var[..., (h - padsize):, :]
        # var_w_cut = var[..., :, (w - padsize):]

        # y_h_cut = self.cut_h(x_h_cut, h, w, h_cut, w_cut, padsize, shave, scale, batchsize, var_h_cut)
        # y_w_cut = self.cut_w(x_w_cut, h, w, h_cut, w_cut, padsize, shave, scale, batchsize, var_w_cut)
        ...
        y_h_top, m_h_top_list = self.cut_h(x_h_top, h, w, h_cut, w_cut, padsize, shave, scale, batchsize) # , var_h_top
        y_w_top, m_w_top_list = self.cut_w(x_w_top, h, w, h_cut, w_cut, padsize, shave, scale, batchsize) # , var_w_top

        x_unfold = x_unfold.view(x_unfold.size(0), -1, padsize, padsize)
        # var_unfold = var_unfold.view(var_unfold.size(0), -1, padsize, padsize)
        ...
        x_unfold
        for i in range(x_range):
            # _, sr = self.forward(x_unfold[i * batchsize:(i + 1) * batchsize, ...], var_unfold[i * batchsize:(i + 1) * batchsize, ...])
            # sr, mask = self.model.forward(x_unfold[i * batchsize:(i + 1) * batchsize, ...])
            sr, mask = self.model(x_unfold[i * batchsize:(i + 1) * batchsize, ...])
            # sr, mask = P.data_parallel(self.model, x_unfold[i * batchsize:(i + 1) * batchsize, ...], range(self.n_GPUs))
    ...
    def cut_h(self, x_h_cut, h, w, h_cut, w_cut, padsize, shave, scale, batchsize): # , var_h_cut

        x_h_cut_unfold = torch.nn.functional.unfold(x_h_cut, padsize, stride=int(shave / 2)).transpose(0, 2).contiguous()
        # var_h_cut_unfold = torch.nn.functional.unfold(var_h_cut, padsize, stride=int(shave / 2)).transpose(0,
        #                                                                                                2).contiguous()

        x_h_cut_unfold = x_h_cut_unfold.view(x_h_cut_unfold.size(0), -1, padsize, padsize)
        # var_h_cut_unfold = var_h_cut_unfold.view(var_h_cut_unfold.size(0), -1, padsize, padsize)
        ...
        x_h_cut_unfold.cuda()
        for i in range(x_range):
            # _, sr = self.forward(x_h_cut_unfold[i * batchsize:(i + 1) * batchsize, ...], var_h_cut_unfold[i * batchsize:(i + 1) * batchsize, ...])
            # sr, mask = self.model.forward(x_h_cut_unfold[i * batchsize:(i + 1) * batchsize, ...])
            sr, mask = self.model(x_h_cut_unfold[i * batchsize:(i + 1) * batchsize, ...])
            # sr, mask = P.data_parallel(self.model, x_h_cut_unfold[i * batchsize:(i + 1) * batchsize, ...], range(self.n_GPUs))
    ...
    def cut_w(self, x_w_cut, h, w, h_cut, w_cut, padsize, shave, scale, batchsize): # , var_w_cut

        x_w_cut_unfold = torch.nn.functional.unfold(x_w_cut, padsize, stride=int(shave / 2)).transpose(0,
                                                                                                       2).contiguous()
        # var_w_cut_unfold = torch.nn.functional.unfold(var_w_cut, padsize, stride=int(shave / 2)).transpose(0,
        #                                                                                                2).contiguous()

        x_w_cut_unfold = x_w_cut_unfold.view(x_w_cut_unfold.size(0), -1, padsize, padsize)
        # var_w_cut_unfold = var_w_cut_unfold.view(var_w_cut_unfold.size(0), -1, padsize, padsize)
        ...
        x_w_cut_unfold.cuda()
        for i in range(x_range):
            # _, sr = self.forward(x_w_cut_unfold[i * batchsize:(i + 1) * batchsize, ...], var_w_cut_unfold[i * batchsize:(i + 1) * batchsize, ...])
            # sr, mask = self.model.forward(x_w_cut_unfold[i * batchsize:(i + 1) * batchsize, ...])
            sr, mask = self.model(x_w_cut_unfold[i * batchsize:(i + 1) * batchsize, ...])
            # sr, mask = P.data_parallel(self.model, x_w_cut_unfold[i * batchsize:(i + 1) * batchsize, ...],
            #                            range(self.n_GPUs))
